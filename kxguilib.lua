...
local function createList(option, parent, holder)
	...
	function option:AddValue(value, displayvalue)
...
local function getFnctions(parent)
	...
	function parent:AddList(option)
		...
		--option.displayed = option.displayed or tostring(option.value)
		option.callback = typeof(option.callback) == "function" and option.callback or function() end
		option.open = false
		option.type = "list"
		option.position = #self.options
		option.flag = option.flag or option.text
		library.flags[option.flag] = tostring(option.displayed)--option.value
		table.insert(self.options, option)
...
function library:Init(nmt)
	...
	if nmt then
		print(1)
		old = game:GetService("CoreGui")["KXGui"]:FindFirstChild(nmt)
		if old then
			old:Destroy()
		end
		self.base.Name = nmt
		self.base.Parent = game:GetService("CoreGui").KXGui
	else 
		print(2)
		old = game:GetService("CoreGui"):FindFirstChild("KXGui")
		if old then
			old:Destroy()
		end
		self.base.Name = "KXGui"
		self.base.Parent = game:GetService("CoreGui")
	end
